...
ner2id = json.load(open('./dataset/docred/ner2id.json', 'r'))
fin_rel2id = json.load(open('./meta/fin_rel2id.json', 'r'))
...
class ReadDataset:
    ...
    def read(self, file_in: str):
        ...
        elif self.dataset == 'fin':
            return read_fin(self.transformers, file_in, save_file, self.tokenizer, self.max_seq_Length)
        else:
...
def read_docred(transfermers, file_in, save_file, tokenizer, max_seq_length=1024):
    use_old_file = True
    if os.path.exists(save_file) and use_old_file:
        ...
        for sample in tqdm(data, desc="Example"):
                ...
                for i_t, token in enumerate(sent):
                    ...
                    if (i_s, i_t) in entity_start:
                        ...
                        if transfermers == 'bert':
                            ...
                            special_token = ['[unused' + str(special_token_i) + ']']  # '[unused1]' '[unused3]'..... '[unused11]'
                        else:
                            special_token = ['*']
                    ...
                    if (i_s, i_t) in entity_end:
                        ...
                        if transfermers == 'bert':
                            ...
                            special_token = ['[unused' + str(special_token_i) + ']']  # '[unused51]' '[unused53]'..... '[unused61]'
                        else:
                            special_token = ['*']
            ...
            ent_sent_ids = []
            ent_types = []
            entity_pos = []  # 每个实体的mentions的分词 (起始,结束)位置
            for e in entities:  # "vertex":
                ent_sent_ids.append(set([]))
                entity_pos.append([])
                mention_num = len(e)
                for m in e:
                    start = sent_map[m["sent_id"]][m["pos"][0]]  # 第sent_id句子的第["pos"][0]个词的 现在的分词位置（实体前后加了标记后的位置）
                    end = sent_map[m["sent_id"]][m["pos"][1]]
                    entity_pos[-1].append((start, end,))
                    ent_sent_ids[-1].add(m["sent_id"])
                ent_types.append(ner2id[e[0]['type']])


            inter_flag = []
            for i, ei in enumerate(ent_sent_ids):
                temp = []
                for j, ej in enumerate(ent_sent_ids):
                    flag = 1
                    if i == j:
                        temp.append(1)
                    else:
                        ...
                        if flag:
                            ...
                            temp.append(0)
                inter_flag.append(temp)


            relations, hts = [], []  # (1,97)的关系向量，有关系的位置为1; 实体对
            ...
            input_ids = tokenizer.convert_tokens_to_ids(sents)  #input_ids的长度是包含[unused] 和  ##est这种的
...
def read_fin(transfermers, file_in, save_file, tokenizer, max_seq_length=2048):
    use_old_file = True
    if os.path.exists(save_file) and use_old_file:
        with open(file=save_file, mode='rb') as fr:
            features = pickle.load(fr)
        ...
        if transfermers == 'bert':
            # entity_type = ["ORG", "-",  "LOC", "-",  "TIME", "-",  "PER", "-", "MISC", "-", "NUM"]
            entity_type = ["-", "poc", "-",  "num", "-",  "ratio", "-",  "date", "-", "other"]
        ...
        for sample in tqdm(data, desc="Example"):
            ...
            entities = sample['entities']  # list[dicts]
            entity_start, entity_end = [], []
            mention_types = []
            for entity in entities:
                for e in entity:
                    for pos in e['pos']:
                        sent_id = pos[0]
                        entity_start.append((sent_id, pos[1]))
                        entity_end.append((sent_id, pos[2] - 1))
                        mention_types.append(e['type'])
            # 原版（没加简称）实体处理
            # for entity in entities:
            #     for pos in entity['pos']:
            #         sent_id = pos[0]
            #         entity_start.append((sent_id, pos[1]))
            #         entity_end.append((sent_id, pos[2] - 1))
            #         mention_types.append(entity['type'])

            for i_s, sent in enumerate(sample['sentences']):
                new_map = {}  # 一个句子第n个词的分词起始位置
                for i_t, token in enumerate(sent):
                    tokens_wordpiece = tokenizer.tokenize(token)
                    if (i_s, i_t) in entity_start:
            ...
            if len(sents) > max_len:
                max_len = len(sents)
            if len(sents) > 1024:  # 512
                up512_num += 1

            train_triple = {}  # 每个实体对有哪些关系及证据
            if "triples" in sample:
                for triple in sample['triples']:
                    r = int(fin_rel2id[triple['r']])
                    if (triple['h'], triple['t']) not in train_triple:
                        train_triple[(triple['h'], triple['t'])] = [{'relation': r}]
                    else:
                        train_triple[(triple['h'], triple['t'])].append({'relation': r})
            ...
            for entity in entities:
                ...
                for e in entity:
                    for m in e['pos']:
                        start = sent_map[m[0]][m[1]]  # 第sent_id句子的第["pos"][0]个词的 现在的分词位置（实体前后加了标记后的位置）
                        end = sent_map[m[0]][m[2]]
                        entity_pos[-1].append((start, end,))
            # for e in entities:
            #     entity_pos.append([])
            #     mention_num = len(e)
