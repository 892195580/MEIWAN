...
class DocREModel(nn.Module):
    ...
    def get_channel_map(self, sequence_output, entity_as):
        ...
        for b in range(bs):
            ...
            h_att = torch.index_select(entity_atts, 0, index_pair[:, 0])  # 头实体的每个头的注意力
            t_att = torch.index_select(entity_atts, 0, index_pair[:, 1])  # 尾实体的每个头的注意力
            ht_att = (h_att * t_att).mean(1)  # 在注意力头上求平均
            ht_att = ht_att / (ht_att.sum(1, keepdim=True) + 1e-5)  # 不知道是啥
    ...
    def forward(self,
        ...
        if self.channel_type == 'context-based':
            feature_map = self.get_channel_map(sequence_output, entity_as)  # (bs,ne,ne,hs)
            ##print('feature_map:', feature_map.shape)
            attn_input = self.liner(feature_map).permute(0, 3, 1, 2).contiguous()  # (bs, 3, ne, ne)
        ...
        attn_map = self.segmentation_net(attn_input)  # (bs, ne, ne, 256)
