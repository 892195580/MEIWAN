...
class DocREModel2(nn.Module):
    def __init__(self, config, args, model, emb_size=768, block_size=64, num_labels=-1, dropout=0.1):
        ...
        self.dis2idx = torch.zeros(1000)  # , dtype='int64'
        self.dis2idx[1] = 1
        self.dis2idx[2:] = 2
        self.dis2idx[4:] = 3
        self.dis2idx[8:] = 4
        ...
        self.type_embs = nn.Embedding(7, emb_size)
        self.ner2id = ner2id
        ...
        rel_name_lists = ['intra', 'inter', 'e2type']
        ...
        #     rel: dglnn.GraphConv(emb_size, 256, norm='right', weight=False, bias=False)
        #     for rel in rel_name_lists
        # })
    ...
    def get_dis_inputs(self, entity_pos):
        offset = 1 if self.config.transformer_type in ["bert", "roberta"] else 0
        ne = max([len(x) for x in entity_pos])  # 本次bs中的最大实体数
        bs = len(entity_pos)
        dis_inputs = []
            ...
            for i in range(ne):
                for j in range(ne):
                    if _dis_inputs[i, j] < 0:
                        _dis_inputs[i, j] = self.dis2idx[-_dis_inputs[i, j]] + 9
                    else:
    ...
    def get_ent_map(self, x, y, dis_emb):  # x,y:(bs, ne, 768)  dis_emb:(bs, ne, ne, 20)
        ...
        rel_map = self.activation(self.ent_layer(fea_map))
        return rel_map#.permute(0, 2, 3, 1).contiguous()

    def create_graph(self, entity_embs, type_embs, types, inter_flag):
        graphs = []
        nes = [len(x) for x in types]
        for b in range(len(inter_flag)):
            d = defaultdict(list)
            for i in range(len(inter_flag[b])):
                for j in range(len(inter_flag[b])):
                    if i == j:
                        continue
                    else:
                        if inter_flag[b][i][j]:
                            d[('entity', 'inter', 'entity')].append((i, j))
                        else:
                            d[('entity', 'intra', 'entity')].append((i, j))
            for i, type in enumerate(types[b]):
                d[('entity', 'e2type', 'etype')].append((i, type))

            num_nodes_dict = {'entity': len(inter_flag[b]), 'etype': 7}
            g = dgl.heterograph(d, num_nodes_dict=num_nodes_dict)
            # g.nodes['entity'].data['f'] = entity_embs[b][:nes[b]]
            # g.nodes['etype'].data['f'] = type_embs
            graphs.append(g)
        return graphs
    ...
    def forward(self,
        ...
        type_embs =self.type_embs(torch.tensor([x for x in range(len(self.ner2id))]).to(input_ids.device))
        graphs = self.create_graph(entity_embs, type_embs, types, inter_flag)
        dis_inputs = self.get_dis_inputs(entity_pos)
        dis_emb = self.dis_embs(dis_inputs.to(input_ids.device))  # (bs, ne, ne, 20)
        feature_ent = self.get_ent_map(entity_embs, entity_embs, dis_emb)  # (bs,256,ne,ne)
        feature_cont = self.get_channel_map(sequence_output, entity_as)  # (bs,ne,ne,768)
        feature_cont = self.att_layer(feature_cont)    # .permute(0, 3, 1, 2) (bs,ne,ne,768) => (bs,256,ne,ne)
